...
def time_span_m(start, end):
    ...
    return round(duration_in_s/60, 1)


def time_span_h(start, end):
    duration = end - start
    duration_in_s = duration.total_seconds()
    return round(duration_in_s/3600, 1)


def time_span_d(start, end):
    duration = end - start
    duration_in_s = duration.total_seconds()
    return round(duration_in_s/86400, 1)
...
print('*'*30)
# One function with f function inside:
...
def f(start, end):
    duration = end - start
    duration_in_s = duration.total_seconds()
    return round(duration_in_s/{sec}, 1)
'''
